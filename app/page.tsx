...
const VIZ_DESCRIPTIONS: Record<string, { title: string; text: string }> = {
  ...
  limitless: {
    ...
    text: "The Sierpinski Tetrahedron. A 3D Fractal generated by the 'Chaos Game'. It reveals how infinite complexity and self-similarity arise from simple iterative rules."
  },
  calculus: {
  ...
  useEffect(() => {
    ...
    const createParticles = () => {
      ...
      for (let i = 0; i < pCount; i++) {
        ...
        // 10. Limitless (Sierpinski Tetrahedron - Chaos Game)
        ...
        // We simulate the game for 'i' steps for randomness, but consistent per particle
        let limX = 0, limY = 0, limZ = 0;
        // Seed based on i to keep deterministic deterministic structure but 'random' look
        let seed = i;
        for (let step = 0; step < 15; step++) {
          const vIdx = Math.floor((seed * 1664525 + 1013904223) % 4); // LCG pseudo-random
          seed = (seed * 1664525 + 1013904223); // update seed
          limX = (limX + sv[vIdx].x) * 0.5;
          limY = (limY + sv[vIdx].y) * 0.5;
          limZ = (limZ + sv[vIdx].z) * 0.5;
        }
        targets.limitless.push(limX, limY, limZ);
