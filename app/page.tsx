...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const updateLines = () => {
      ...
      if (['lorenz', 'quant', 'series', 'topology', 'calculus', 'astar', 'dijkstra', 'bfs', 'dfs'].includes(mode)) {
    ...
    const animate = () => {
      ...
      if (['astar', 'dijkstra', 'bfs', 'dfs'].includes(mode)) {
        ...
        if (grid) {
          for (let i = 0; i < grid.nodes.length && i < configRef.current.particleCount; i++) {
            ...
            if (!node.walkable) {
              ...
              // Green Frontier
              colors[i3] = 0.0; colors[i3 + 1] = 1.0; colors[i3 + 2] = 0.0;
            } else {
              // Unvisited - Faint White/Gold
        ...
        // Reset Colors for other modes if returning from algo mode? 
        // (Simplified: We just let other modes overwrite colors if they need to, 
        // or we reset them once here. For now, since most other modes just use static colors 
        // assigned at init, we might need to restore them.)
        // Ideally, we'd have a 'restoreColors' flag or function. 
        // Hack: Constantly reset colors for non-algo & non-neural modes?
        if (mode !== 'neural') {
          // Re-apply base colors (Gold/White random) - this is expensive to do every frame 
          // but ensures consistency. Better: Only do it once on mode switch.
          // For this task, assuming 'neural' handles its own colors, others are static.
          // We'll rely on the fact that Algorithm Mode is the only one mutating colors continuously besides Neural.
          // So if we switch OUT of Algorithm, we should reset. 
          // Implemented in `changeVizMode` ideally, but color buffer is here.
          // Let's just do a quick reset if we detect a switch (checking a ref wrapper or similar).
          // For now, I'll add a simple Color Reset in the loop if we aren't in those modes
